"""
Minimal Chainlit frontend for AgencyCoachAI.
Simple therapeutic conversations without complex features.
"""
...
async def on_chat_start():
    ...
    # Welcome message
    welcome_message = """# Welcome to AgencyCoachAI! 🌟
    ...
    try:
        ...
        if os.getenv("AZURE_OPENAI_API_KEY"):
            response = await openai_client.chat.completions.create(
                model=os.getenv("OPENAI_DEPLOYMENT_NAME_REALTIME", "gpt-4o-mini-realtime-preview"),
        ...
        error_message = "I'm having trouble processing your message right now. This might be a temporary issue with the AI service. Please try again in a moment."
        
        # If it's an Azure deployment issue, provide more specific guidance
        if "DeploymentNotFound" in str(e) or "model" in str(e).lower():
            error_message = "It looks like there might be an issue with the AI model configuration. Please check that the deployment name is correct in your environment settings."
